...
- `tests/support/` — Shared test doubles: `FakeNotion` (mutable in-memory Notion store; `discard_writes`/`filter_reads` reproduce eval-runner semantics), `SignalSink` (outbound capture; patches the listener's own namespace for receipts/typing), `shame.BANNED_PATTERNS`. Contract-tested by `tests/unit/test_conversation_fakes.py`; `tests/evals/runner.py` builds its Notion stub on top.
