...
- `tests/support/` — Shared test doubles: `FakeNotion` (mutable in-memory Notion store; `discard_writes`/`filter_reads` reproduce eval-runner semantics), `SignalSink` (outbound capture; patches the listener's own namespace for receipts/typing), `shame.BANNED_PATTERNS`. Contract-tested by `tests/unit/test_conversation_fakes.py`; `tests/evals/runner.py` builds its Notion stub on top. Also `harness.py` (`Conversation` multi-turn driver, entered through `SignalListener` so `thread_id` derivation and the auth gate are exercised) and `invariants.py` (the seven per-turn invariants).
- `tests/e2e/` — Multi-turn conversation scenarios: real compiled graph, real Postgres checkpointer, real LLM via the LiteLLM proxy; Notion and Signal faked. Gated by `ENABLE_E2E_CONVERSATIONS=true`. See `tests/e2e/README.md`.
...
- `docs/python-rewrite/test-rig.md` — Authoritative test rig architecture spec: layer table, 11 bug classes, regression catalog convention, eval fixture format, integration mock discipline, LLM swap mechanism
...
- `.devcontainer/` — Devcontainer definition. `post-create.sh` provisions `.venv` via `uv` (Python 3.12, `-e ".[dev]"`) so a fresh clone can run `pytest tests/unit/` with no manual setup; `devcontainer.json` puts `.venv/bin` on `remoteEnv.PATH` so the `.githooks/pre-commit` gate resolves `pytest` / `ruff` without shell activation. CI's `Devcontainer Build Check` builds the image only — it does not run `postCreateCommand`, so `tests/unit/test_devcontainer_python_env.py` guards the provisioning wiring. The container also inherits the developer's agent setup: `devcontainer.json` mounts the repo at its host path (`workspaceMount`/`workspaceFolder`), and `sync-claude-config.sh` places the host `~/.claude` contents into the container user's home. Two mounts, split on writability: the whole host `~/.claude` read-only (it carries `settings.json`, `hooks/`, and `plugins/`, so repo-controlled container code must not be able to write a hook the host agent later runs), and `~/.claude/projects` writable (Claude Code writes memory, sessions, and plans there as it works). Claude Code names per-project state after the working directory, which is why the repo is mounted at its host path. `tests/unit/test_devcontainer_claude_config.py` guards that wiring.
...
- `.github/workflows/python-validation.yml` — Required CI gate: runs on every PR. The `Python Validation Required` status check always reports. `ruff`, `mypy`, `pytest-unit`, and `pytest-db` run only when the Python change filter matches `app/`, `migrations/`, `tests/`, `scripts/*.py`, or `pyproject.toml`; they are skipped (and treated as success) on non-Python PRs. `pytest-db` runs `tests/integration/` + `tests/regressions/` against a `postgres:16-alpine` service container — no secrets, since every outbound client is faked. The service database must be named `hml`: `migrations/0005_readonly_user.sql` issues a literal `GRANT CONNECT ON DATABASE hml`. `tests/integration/test_db_service_present.py` is the positive control that fails (rather than skips) under CI when the container is missing.
...
- `.github/workflows/e2e.yml` — `pull_request` (always triggered; path filtering via an internal `changes` job using `dorny/paths-filter` on `app/`, `migrations/`, `tests/e2e/`, `tests/support/`, `setup/model-tiers.json`, `pyproject.toml`, `.github/workflows/e2e.yml`) + `workflow_dispatch`. Runs `tests/e2e/` on the self-hosted `homelab` runner; Postgres started with `docker run` in the runner's own network namespace (no `services:` block — service container ports are unreachable from this runner topology): real graph, real checkpointer, real LLM, faked Notion/Signal. Blocking pre-merge check via the `E2E Conversations Required` aggregator — cross-turn state regressions merge clean and surface in production, so a nightly-only run would report them a day late. Shares the `homelab-llm-serial` concurrency group with `nightly-evals.yml` and `model-swap.yml`.
...
- `.github/scripts/review/prompts/test.md` — Test coverage reviewer: enforces 11 test-rig contract clauses on PRs touching app/**, migrations/**, setup/model-tiers.json, app/prompts/**, docs/ai-prompts/**, tests/**, the test reviewer prompt, review schemas, docs/python-rewrite/test-rig.md, and docker/compose.yaml
