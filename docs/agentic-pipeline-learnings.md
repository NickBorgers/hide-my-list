...
- **Fixer-owned SHA re-entry is skipped.** `review-entry.yml` refuses automatic `pull_request` dispatch when reviewed SHA has both a `review/pipeline` status whose description contains `fixer post-push claim` and a finalized `review/verdict` status. This catches both cases: pending-only dedup handles the event while the fixer claim is active, and the status-history guard handles the event after cleanup/finalize advances the latest claim to terminal. Requiring `review/verdict` keeps a pipeline failure after push-but-before-finalize retryable. `/review` comments bypass this guard so humans can force a re-review of the exact SHA.
